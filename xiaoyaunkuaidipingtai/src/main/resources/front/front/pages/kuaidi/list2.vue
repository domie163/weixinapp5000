	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('kuaidi','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                 ...
                 <view v-show="item.kuaidiUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">快递编号</text>
                     <text style="width: 70%">{{item.kuaidiUuidNumber}}</text>
                 </view>
                 <view v-show="item.kuaidiValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
...
<script>
	export default {
		...
		async onShow() {
			...
			kuaidiTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
            this.categoryList = kuaidiTypes.data.list
				...
				let params = {
                    ...
                    yonghuId:this.userId,//查询自己的
							...
							if (this.categoryName != 0) {
                                params.kuaidiTypes = this.categoryName
                            }
