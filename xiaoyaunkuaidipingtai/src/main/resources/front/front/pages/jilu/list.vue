	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"16rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.jiluName" type="text" placeholder="快递记录名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('jilu','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('jilu','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='jiluTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in jiluTypesList" :key="index" :class="jiluTypes===item.codeIndex?'tabActive':''"
                                  @tap="jiluTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.jiluName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('jilu','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <view v-show="item.kuaidiName" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">快递名称</text>
                    <text style="width: 70%">{{item.kuaidiName}}</text>
                </view>
                <view v-show="item.kuaidiTypes" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">快递类型</text>
                    <text style="width: 70%">{{item.kuaidiValue}}</text>
                </view>
                ...
                <!-- 当前表 -->
                 <view v-show="item.jiluUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">快递记录编号</text>
                     <text style="width: 70%">
                         <text v-if="item.jiluUuidNumber != null">{{item.jiluUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.jiluFachuAddress" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">发出地</text>
                     <text style="width: 70%">
                         <text v-if="item.jiluFachuAddress != null">{{item.jiluFachuAddress}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.jiluJieshouAddress" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">接受地</text>
                     <text style="width: 70%">
                         <text v-if="item.jiluJieshouAddress != null">{{item.jiluJieshouAddress}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.jiluValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">快递记录类型</text>
                     <text style="width: 70%">{{item.jiluValue}}</text>
                 </view>
                <view v-show="item.jiluContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">快递记录介绍</text>
                    <view style="width: 70%" v-html="item.jiluContent"></view>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('jilu','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('jilu','审核') && item.jiluYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 快递记录类型 字段 字典表查询方法
            async jiluTypesSelect(){
                let jiluTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'jilu_types',
                });
                jiluTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.jiluTypesList = jiluTypes.data.list
            },
            //当前表的 快递记录类型字段类别搜索
			jiluTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.jiluTypes != 0) {
                    params.jiluTypes = this.jiluTypes
                }
                let _this = this
                let res = await this.$api.page(`jilu`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('jilu', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`jilu`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.jiluName) {
                    searchForm['jiluName'] = this.searchForm.jiluName
                }

                let res = await this.$api.page(`jilu`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.jiluContent!=null){
                        item.jiluContent = item.jiluContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
