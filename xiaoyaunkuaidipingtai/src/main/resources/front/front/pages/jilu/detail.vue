	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('jilu', this.jiluId);
                    this.detail = res.data
					this.detail.jiluContent = this.detail.jiluContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.jiluPhoto ? this.detail.jiluPhoto.split(",") : [];
				},
