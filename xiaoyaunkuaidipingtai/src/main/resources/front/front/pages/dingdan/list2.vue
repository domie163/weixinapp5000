	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('dingdan','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.dingdanName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('dingdan','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                 <view v-show="item.dingdanUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">订单编号</text>
                     <text style="width: 70%">{{item.dingdanUuidNumber}}</text>
                 </view>
                 <view v-show="item.buyNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">购买数量</text>
                     <text style="width: 70%">{{item.buyNumber}}</text>
                 </view>
                 <view v-show="item.dingdanWupin" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">购买物品</text>
                     <text style="width: 70%">{{item.dingdanWupin}}</text>
                 </view>
                 <view v-show="item.dingdanDizhi" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">收货地址</text>
                     <text style="width: 70%">{{item.dingdanDizhi}}</text>
                 </view>
                 <view v-show="item.dingdanTruePrice" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">实付价格</text>
                     <text style="width: 70%">{{item.dingdanTruePrice}}</text>
                 </view>
                 <view v-show="item.dingdanCourierNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">快递单号</text>
                     <text style="width: 70%">{{item.dingdanCourierNumber}}</text>
                 </view>
                 <view v-show="item.dingdanValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">订单类型</text>
                     <text style="width: 70%">{{item.dingdanValue}}</text>
                 </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 12rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('dingdan','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                ...
                <!--<view v-if="isAuth('dingdan','审核') && item.dingdanYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
		  	...
		  	let dingdanTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'dingdan_types'
		  	});
		  	this.dingdanTypesList = dingdanTypes.data.list;
			dingdanTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
            this.categoryList = dingdanTypes.data.list
		...
		methods: {
				...
				let params = {
                    ...
                    yonghuId:this.userId,//查询自己的
							...
							if (this.categoryName != 0) {
                                params.dingdanTypes = this.categoryName
                            }
            ...
            wuyong(id) {
                ...
                uni.showModal({ content: '确定要 么?',title: '提示',
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.update(`dingdan`, {
                                id:id,
                            });
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('dingdan', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
							_this.mescroll.resetUpScroll()
						}
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`dingdan`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.dingdanUuidNumber) {
                    searchForm['dingdanUuidNumber'] = this.searchForm.dingdanUuidNumber
                }

                let res = await this.$api.list(`dingdan`, searchForm);
				// 如果是第一页数据置空
