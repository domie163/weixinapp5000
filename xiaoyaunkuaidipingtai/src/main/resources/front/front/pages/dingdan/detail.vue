	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('dingdan', this.dingdanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.dingdanPhoto ? this.detail.dingdanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('dingdanYuyue', {
